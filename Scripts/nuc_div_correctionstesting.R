test <- readDNAStringSet("~/Documents/PhD/PhD/RNA_IGR/Isolate_Igr_Data/igrs/final_igrs_up.xmfa")
test1 <- test[17737:17744]
test1 <- as.DNAbin(AlignSeqs(test1))
nuc.div(test1)

###Script to quantify nucleotide diversity between the 8 isolates for each igr for each locus
#CRITICAL - YOU DONT ALIGN THE SEQUENCES BEFORE CALCULATING PI SO SINGLE INSERTIONS RUIN EVERYTHING
###APPROACH
...
for(i in seq(1,length(up_igrs), 8)){
  ...
  testlocus_up <- as.DNAbin(AlignSeqs(testlocus_up))
  testlocus_down <- as.DNAbin(AlignSeqs(testlocus_down))
...
write.xlsx(pi_values, "~/Documents/PhD/PhD/RNA_IGR/Isolate_Igr_Data/pi_valuesalign.xlsx")
...
for(i in 1:length(pi_values$Locus)){
  ...
  if(pi_values$pi_up[i] > 1.5*mean_total && pi_values$pi_down[i] > mean_total){
    pi_values$Var[i] <- 'high'
  }
  if(pi_values$pi_down[i] > 1.5*mean_total && pi_values$pi_up[i] > mean_total){
    pi_values$Var[i] <- 'high'
  }
}
table(pi_values$Var)
