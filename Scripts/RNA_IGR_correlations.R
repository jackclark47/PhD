...
log2hitsfilt <- log2hits[-c(1),-c(1:8, 10, 11, 36:107)]

#Remove entries which don't have an NEIS code
log2hitsfilt <- log2hitsfilt[!is.na(log2hitsfilt$`1717.genes`),]
...
plotdata <- as.data.frame(matrix(nrow=1717,ncol=4))
alleleplotdata <- as.data.frame(matrix(nrow=1717,ncol=4))
...
write.xlsx(stats_summary, 'IGR.Groups.Statsummarytest.xlsx')
