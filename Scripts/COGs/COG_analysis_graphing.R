...
setwd("~/Documents/PhD/PhD/RNA_IGR/eggNOG_annotations/Isolates")
...
#load rna loci
rna <- read.xlsx("~/Documents/PhD/PhD/RNA_IGR/11-7_run_5_transcripts-cdb-16082022.xlsx", sheet=9)
rna <- rna[which(rna$`log2(EXP/Min)` >=1),]
rnalog4 <- rna[which(rna$`log2(EXP/Min)` >=2),]
#load distributions of cogs in all loci
cogs <- read.xlsx('~/Documents/PhD/PhD/RNA_IGR/eggNOG_annotations/all_coding_loci.annotations.xlsx', startRow = 3)[1:1765,]

#match loci in the two lists using log2fold change >1 and >2 as cutoffs
cogslog2 <- cogs[which(cogs$query %in% rna$`1717.genes`),] 
length(unique(rna$`1717.genes`)) #590 loci, of which 404 have cog information
cogslog4 <- cogs[which(cogs$query %in% rnalog4$`1717.genes`),] 
length(unique(rnalog4$`1717.genes`)) #105 loci, of which 76 have cog information
cogsunderlog2 <- cogs[which(!(cogs$query %in% rna$`1717.genes`)),]
cogsunderlog2 <- cogsunderlog2[which(!(cogsunderlog2$query %in% rnalog4$`1717.genes`)),]
...
#for rna data
rna_counts <- read.xlsx("~/Documents/PhD/PhD/RNA_IGR/eggNOG_annotations/rna_cogs.xlsx")[1:22, c(3,4,6,9)]
...
#for igr data
igr_counts <- read.xlsx("~/Documents/PhD/PhD/RNA_IGR/eggNOG_annotations/igr_cogs.xlsx")[1:22,-c(1,2, 10:16)]
...
corrplot(rna_chisq$residuals, method = 'color', is.cor = FALSE, tl.cex = 1, tl.col = 'black', 
         cl.ratio = 0.5,
         cl.align.text = 'l',
         cl.offset = 0.2)

igr_chisq <- chisq.test(igr_counts[-c(11),c(1:5)])
igr_chisq$p.value
...
isolate_cogs <- read.xlsx("~/Documents/PhD/PhD/RNA_IGR/eggNOG_annotations/eggNOG_COG_summaries.xlsx")[1:22,9:16]
